...
export default function Chat() {
    ...
    const [selectedChatId, setSelectedChatId] = useState(null); // 선택된 채팅 정보의 id 저장
    ...
    return createPortal(
        <ChatContainer>
            {selectedChatId ? (
                <ChatDetail chatId={selectedChatId} onBack={() => setSelectedChatId(null)} />
            ) : (
                <>
                    <ChatTitle>채팅</ChatTitle>
                    {chatList.map((chat) => (
                        <ChatCard key={chat.id} onClick={() => setSelectedChatId(chat.id)}>
                            <AvatarWrapper>
                                {chat.src ? (
